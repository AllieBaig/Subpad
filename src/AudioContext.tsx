...
export function AudioProvider({ children }: { children: ReactNode }) {
  ...
  const { settings, updateSubliminalSettings, updateSettings, updateSessionState } = useSettings();
  ...
  // Save Session State
  useEffect(() => {
    updateSessionState({ isPlaying, lastActiveAt: Date.now() });
  }, [isPlaying, updateSessionState]);
    ...
    async function loadData() {
      try {
        ...
        if (isMounted) {
          ...
          // Auto-resume session if enabled
          const stability = settings.advancedAudio?.stabilityEngine;
          const autoRestore = stability?.sessionMemory?.isEnabled && stability?.sessionMemory?.autoRestore;
          const wasPlaying = settings.sessionState?.isPlaying;
          
          if (autoRestore && wasPlaying) {
            console.log("[AudioContext] Session Memory Restore: Resuming playback");
            setTimeout(() => {
              if (isMounted) setIsPlaying(true);
            }, 1000); // Small delay to ensure engines are wired
          }
