...
const HZ_PROFILES_STORE = 'hz_profiles';
  ...
  try {
    return await openDB(DB_NAME, DB_VERSION, {
      upgrade(db, oldVersion, newVersion, transaction) {
        ...
        if (oldVersion < 5) {
          if (!db.objectStoreNames.contains(HZ_PROFILES_STORE)) {
            db.createObjectStore(HZ_PROFILES_STORE, { keyPath: 'id' });
          }
        }
      },
      blocking() {
...
// Hz Profiles
export async function saveHzProfile(profile: any) {
  try {
    const db = await initDB();
    await db.put(HZ_PROFILES_STORE, profile);
  } catch (err) {
    handleDBError(err, 'saveHzProfile');
  }
}

export async function getHzProfiles(): Promise<any[]> {
  try {
    const db = await initDB();
    return db.getAll(HZ_PROFILES_STORE);
  } catch (err) {
    console.error("Failed to retrieve hz profiles:", err);
    return [];
  }
}

export async function deleteHzProfile(id: string) {
  try {
    const db = await initDB();
    await db.delete(HZ_PROFILES_STORE, id);
  } catch (err) {
    console.error("Failed to delete hz profile:", err);
  }
}
